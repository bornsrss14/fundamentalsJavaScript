...
//large form
for (const movement of arryMovements) {
  if(movement > 0){
    console.log(`Usted hizo un deposito de ${movement}`);
  } else{
    console.log(`Usted hizo un retiro de ${Math.abs(movement)}`);
  }
}
...
for (const mov of arryMovements) {
  const result = mov > 0 ? console.log(`Deposito de ${mov}`) : console.log(`retiro de ${Math.abs(mov)}`);
}